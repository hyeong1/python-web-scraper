...
def get_pages(url):
    ...
    pages = len(
        soup.find("ul", class_="bsj-nav").find_all(class_="page-numbers")[0:-1])
    return pages
...
def scrap_skills(url):
    ...
    soup = BeautifulSoup(response.content, "html.parser")
    skills = soup.find("section").find(
        "div", id="skills-list").find("ul").find_all("li")
    for skill in skills:
        ...
        skill = skill.replace(" ", "-")
        skill = skill.replace("/", "")
        if skill == "marketing" or skill == "management":
            skill = skill + "-2"  # marketing url change
        if not (skill == "engineering"):
